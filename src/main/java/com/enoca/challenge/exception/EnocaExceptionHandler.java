...
public class EnocaExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleJobException(WebRequest webRequest, EnocaException enocaException) {
        ProblemDetail problemDetail = ProblemDetail.forStatusAndDetail(enocaException.getStatus(), enocaException.getMessage());
        problemDetail.setInstance(URI.create(webRequest.getDescription(false)));
        problemDetail.setTitle(enocaException.getClass().getSimpleName());
        problemDetail.setStatus(enocaException.getStatus().value());
        problemDetail.setProperty(TIMESTAMP, LocalDateTime.now());
        return new ResponseEntity<>(problemDetail, enocaException.getStatus());
    }
